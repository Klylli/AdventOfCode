...
int main(int argc, char** argv){
	...
	long sum = 0;
	int ticketNumber = 1;
	int winningNumsInTicket[196];
	for(int i = 0; i < 196; i++){
		winningNumsInTicket[i] = 1;
	}
...
//		printArray(myNumbers, numbersMine);
		...
		for(int j = ticketNumber; j < 196 && j < ticketNumber + curr; j++){
			winningNumsInTicket[j] += (winningNumsInTicket[ticketNumber -1]);
	...
	for(int i = 0; i < 196; i++){
		sum+= (long)winningNumsInTicket[i];
	}
