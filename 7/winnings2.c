...
/*
   void getPermutations(char** perms, char* hand, int jokers, int idx){

   }

   char getBestType(char* s, int length){
   int jCounter = 0;
   ...
   if(jCounter > 0) return getType(s, length);
   int permutations = 1;
   for(int i = 0; i < jCounter; i++) permutations*=13;
   char** perms = malloc(permutations * sizeof(char*));
   for(int i = 0; i < permutations; i++){
   perms[i] = malloc(5);
   }
   char* types = malloc(permutations);
   getPermutations(perms, s, jCounter, permutations);
   for(int i = 0; i < permutations; i++){
   if(types[i] == '5') return '5';
   }
   for(int i = 0; i < permutations; i++){
   if(types[i] == '4') return '4';
   }
   for(int i = 0; i < permutations; i++){
   if(types[i] == 'f') return 'f';
   }
   for(int i = 0; i < permutations; i++){
   if(types[i] == '3') return '3';
   }
   for(int i = 0; i < permutations; i++){
   if(types[i] == '2') return '2';
   }
   for(int i = 0; i < permutations; i++){
   if(types[i] == '1') return '1';
   }
   return 'h';
   }
   */
...
int main(int argc, char **argv) {
  ...
  while (getline(&line, &len, file) != -1) {
    ...
    // cards[amount].type = getBestType(cards[amount].hand, numberOfCards);
    printf("%c\n", cards[amount].type);
    hands[amount] = line;
    amount++;
    line = NULL;
